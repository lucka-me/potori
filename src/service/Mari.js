...
class Parser {
    static mail(fullMail, keys) {
        const portal = { status: StatusKit.types.get(keys.type).code, };
    ...
    static reason(mailBody, scanner) {
        ...
        const undeclaredCode = StatusKit.reasons.get('undeclared').code;
        let reason = undeclaredCode;
        for (const reason of StatusKit.reasons.values()) {
            for (const keyword of reason.keyword[scanner]) {
                if (mainBody.search(keyword) > -1) {
                    reason = reason.code;
                    break;
                }
            }
            if (reason !== undeclaredCode) break;
        }
        return reason;
    }
...
class Mari {
    ...
    getListRequest(pageToken, keys) {
        return gapi.client.gmail.users.messages.list({
            'userId': 'me',
            'q': StatusKit.types.get(keys.type).query[keys.scanner],
            'pageToken': pageToken
        });
    }
    ...
    handleListRequest(response, keys, list) {
        if (response.result.messages) list.push(...response.result.messages);
            if (response.result.nextPageToken) {
                ...
                this.event.bufferUpdate(this.progress.list / 6);
