...
class DetailsDialogMap extends DialogPrototype {
    ...
    search() {
        ...
        const onFailed = () => {
            ...
            ui.dialog.alert.open(value.string.alert.queryLngLatFailed);
            this.button.search.root_.disabled = false;
        }
        this.button.search.root_.disabled = true;
        this.bs.queryLngLat(
            ui.dialog.details.portal.id, onSuccess, onFailed
        );
    }
...
class DetailsDialog extends DialogPrototype {
    ...
    closed(event) {
        ...
        if (selectedStatus !== 'pending') {
            ...
            if (!time) {
                ui.dialog.alert.open(value.string.alert.invalidDateTime);
                return;
            }
        ...
        if (shouldUpdate) {
            if (ui.dialog.details.selectedStatus !== 'rejected') {
                this.portal.status = value.data.type[ui.dialog.details.selectedStatus].code;
            } else {
                this.portal.status = value.data.reason[this.selectReason.value].code;
            }
            const card = document.getElementById(`card-${this.portal.id}`);
    ...
    open(portal) {
        ...
        if (type === 'pending') {
            process.bs.query(portal.id, (data) => {
