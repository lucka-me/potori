...
class DetailsDialogMap extends UIKitPrototype {
    ...
    search() {
        ...
        const onFailed = () => {
            if (!this.dialog.isOpen) return;
            AlertDialog.open(value.string.alert.queryLngLatFailed);
            this.button.search.root_.disabled = false;
        }
...
class DetailsDialog extends DialogPrototype {
    ...
    closed(event) {
        ...
        if (selectedStatus !== 'pending') {
            ...
            if (!time) {
                AlertDialog.open(value.string.alert.invalidDateTime);
                return;
            }
        ...
        if (shouldUpdate) {
            if (this.selectedStatus !== 'rejected') {
                this.portal.status = value.data.type[this.selectedStatus].code;
            } else {
                this.portal.status = value.data.reason[this.selectReason.value].code;
            }
            ui.update(this.portal);
        }
    }
    ...
    updateStyle() {
        ...
        this.map.ctrl.setStyle(
            value.string.mapbox.style[Dark.enabled ? 'dark' : 'default']
        );
    }
}

export default new DetailsDialog();