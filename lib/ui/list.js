...
class ListView extends UIKitPrototype {
    ...
    updateVisibility(portal, card) {
        ...
        if (type === 'rejected') {
            ...
            card.hidden = !dashboard.filter.reason[reason].checked;
        } else {
            card.hidden = !dashboard.filter.type[type].checked;
        }
    }
    ...
    buildCard(portal) {
        ...
        actionLocation.listen('click', () => {
            dashboard.map.ctrl.easeTo({ center: portal.lngLat, zoom: 16 })
        });
