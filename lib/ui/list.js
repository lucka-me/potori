...
class ListView extends UIKitPrototype {
    ...
    updateVisibility(portal, card) {
        ...
        if (type === 'rejected') {
            ...
            card.hidden = !ui.dashboard.filter.reason[reason].checked;
        } else {
            card.hidden = !ui.dashboard.filter.type[type].checked;
        }
    }
    ...
    buildCard(portal) {
        ...
        primaryAction.listen('click', () => ui.dialog.details.open(portal));
        ...
        if (versionKit.fullFeature) {
            ...
            actionStatus.listen('click', () => {
                ...
                ui.dialog.alert.open(`Brainstorming ID copied: ${portal.id}`);
            });
        }
        ...
        actionLocation.listen('click', () => {
            ui.dashboard.map.ctrl.easeTo({ center: portal.lngLat, zoom: 16 })
        });
