dashboard.map = {
    ...
    loaded() {
        //return this.ctrl && this.ctrl.loaded();
    ...
    update() {
        ...
        this.updateSource(
            'accepted',
            this.generateGeoJSON([ value.data.type.accepted.code ])
        );
        this.updateSource(
            'pending',
            this.generateGeoJSON([ value.data.type.pending.code ])
        );
    },
    updateRejected() {
        const codes = [];
        for (const key of Object.keys(dashboard.filter.reason)) {
            if (!dashboard.filter.reason[key].checked) continue;
            codes.push(value.data.reason[key].code);
        }
        this.updateSource('rejected', this.generateGeoJSON(codes));
    },
    ...
    updateSource(type, data) {
        ...
        this.ctrl.on('click', `${id}-unclustered`, event => {
            ...
            new mapboxgl.Popup()
                ...
                .addTo(this.ctrl);
            ui.list.root.scrollTo(
                0,
                document.getElementById(`card-${event.features[0].properties.id}`).offsetTop - ui.list.root.offsetTop - 8);
        });
    },
    updateStyle() {
        if (!dashboard.map.loaded()) return;
        this.ctrl.setStyle(value.string.mapbox.style[ui.dark.enabled ? 'dark' : 'default']);
