...
var consoleReceivedRow = document.getElementById("consoleReceivedRow");
var consoleAcceptedRow = document.getElementById("consoleAcceptedRow");
var consoleRejectedRow = document.getElementById("consoleRejectedRow");
var consolePendingRow = document.getElementById("consolePendingRow");
var consoleReceivedCount = document.getElementById("consoleReceivedCount");
var consoleAcceptedCount = document.getElementById("consoleAcceptedCount");
var consoleRejectedCount = document.getElementById("consoleRejectedCount");
var consolePendingCount = document.getElementById("consolePendingCount");
var consoleCheckShowAccepted = document.getElementById("consoleCheckShowAccepted");
var consoleCheckShowRejected = document.getElementById("consoleCheckShowRejected");
var consoleCheckShowPending = document.getElementById("consoleCheckShowPending");
var consoleCollapseAll = document.getElementById("consoleCollapseAll");
...
function updateAuthStatus(isSignedIn) {
    if (isSignedIn) {
        ...
        consoleStatus.innerHTML = "Please authorize first."
        consoleStatus.hidden = false;
        consoleReceivedRow.hidden = true;
        consoleAcceptedRow.hidden = true;
        consoleRejectedRow.hidden = true;
        consolePendingRow.hidden = true;
        consoleCheckShowAccepted.disabled = true;
        consoleCheckShowRejected.disabled = true;
        consoleCheckShowPending.disabled = true;
...
function processChecking() {
    ...
    consoleStatus.innerHTML = "Processing mails...";
...
function requestConfirmationMailList() {
    consoleReceivedRow.hidden = false;
    consoleReceivedCount.innerHTML = "Loading...";
    requestMailList(
        "from:ingress-support@nianticlabs.com Portal submission confirmation -edit -photo",
        function(messages) {
            mailConfirmationList = messages;
            consoleReceivedCount.innerHTML = mailConfirmationList.length;
...
function requestAcceptanceMailList() {
    consoleAcceptedRow.hidden = false;
    consoleAcceptedCount.innerHTML = "Loading...";
    requestMailList(
        "from:ingress-support@nianticlabs.com Portal review complete now available -edit -photo",
        function(messages) {
            mailAcceptanceList = messages;
            consoleAcceptedCount.innerHTML = mailAcceptanceList.length;
...
function requestRejectionMailList() {
    consoleRejectedRow.hidden = false;
    consoleRejectedCount.innerHTML = "Loading...";
    requestMailList(
        "from:ingress-support@nianticlabs.com Portal review complete reviewed -edit -photo",
        function(messages) {
            mailRejectionList = messages;
            consoleRejectedCount.innerHTML = mailRejectionList.length;
    ...
    var requestHandler = function(response) {
        ...
        if (response.result.nextPageToken) {
            ...
            if (mailConfirmationList && mailAcceptanceList && mailRejectionList) {
                ...
                consolePendingRow.hidden = false;
                consolePendingCount.innerHTML = pendingMailCount;
            }
        }
    };
...
function processConfirmationMails() {
    portalPendingList = [];
    consoleReceivedCount.innerHTML = "0/" + mailConfirmationList.length;
    processMails(
        mailConfirmationList, portalPendingList,
        function(length) {
            consoleReceivedCount.innerHTML = length + "/" + mailConfirmationList.length;
        }
    );
}
...
function processAcceptanceMails() {
    portalAcceptedList = [];
    consoleAcceptedCount.innerHTML = "0/" + mailAcceptanceList.length;
    processMails(
        mailAcceptanceList, portalAcceptedList,
        function(length) {
            consoleAcceptedCount.innerHTML = length + "/" + mailAcceptanceList.length;
        },
        getIntelAndLngLat
    );
}
...
function processRejectionMails() {
    portalRejectedList = [];
    consoleRejectedCount.innerHTML = "0/" + mailRejectionList.length;
    processMails(
        mailRejectionList, portalRejectedList,
        function(length) {
            consoleRejectedCount.innerHTML = length + "/" + mailRejectionList.length;
        },
...
function displayPortals() {
    ...
    consoleStatus.innerHTML = "Process finished.";

    consoleAcceptedCount.innerHTML = portalAcceptedList.length == 0 ? "0/0(0%)" : (portalAcceptedList.length + "/" + portalAcceptedList.length + "(" + (portalAcceptedList.length / mailConfirmationList.length * 100).toFixed(2) + "%)");
    consoleRejectedCount.innerHTML = portalRejectedList.length == 0 ? "0/0(0%)" : (portalRejectedList.length + "/" + portalRejectedList.length + "(" + (portalRejectedList.length / mailConfirmationList.length * 100).toFixed(2) + "%)");
    consolePendingCount.innerHTML = portalPendingList.length == 0 ? "0/0(0%)" : (portalPendingList.length + "/" + portalPendingList.length + "(" + (portalPendingList.length / mailConfirmationList.length * 100).toFixed(2) + "%)");

    consoleCheckShowAccepted.disabled = false;
    consoleCheckShowRejected.disabled = false;
    consoleCheckShowPending.disabled = false;
    consoleCollapseAll.style.display = "inline";
}
...
/* BEGIN: Events for Console */

function onChangeShowAccepted() {
    changeShow(portalAcceptedList, consoleCheckShowAccepted.checked);
}

function onChangeShowRejected() {
    changeShow(portalRejectedList, consoleCheckShowRejected.checked);
}

function onChangeShowPending() {
    changeShow(portalPendingList, consoleCheckShowPending.checked);
}
...
function onClickCollapseAll() {
    var isHidden = consolePendingRow.hidden;
    consoleAcceptedRow.hidden = !isHidden;
    consoleRejectedRow.hidden = !isHidden;
    consolePendingRow.hidden = !isHidden;
    if (isHidden) {
        consoleCollapseAll.className = "cursor-pointer fas fa-angle-double-up";
    } else {
        consoleCollapseAll.className = "cursor-pointer fas fa-angle-double-down";
    }
}

/* END: Events for Console */