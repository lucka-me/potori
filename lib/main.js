...
const process = {
    ...
    redacted: {
        status: {
            confirmation: { finished: false },
            acceptance: { finished: false },
            rejection: { finished: false },
        },
        start: function() {
            process.redacted.confirmation();
            process.redacted.acceptance();
            process.redacted.rejection();
        },
        confirmation: function() {
            let count = 0;
            process.mails(
                { scanner: value.string.key.redacted, type: value.string.key.confirmation },
                function(fullMail) {
                    portalList.push(process.parse.mail(fullMail));
                    count += 1;
                    return count;
                }
            );
        ...
        acceptance: function() {
            let count = 0;
            process.mails(
                { scanner: value.string.key.redacted, type: value.string.key.acceptance },
                function(fullMail) {
                    portalList.push(process.parse.mail(fullMail, process.redacted.parse.acceptance));
                    count += 1;
                    return count;
                }
            );
        ...
        rejection: function() {
            let count = 0;
            process.mails(
                { scanner: value.string.key.redacted, type: value.string.key.rejection },
                function(fullMail) {
                    portalList.push(process.parse.mail(fullMail, process.redacted.parse.rejection));
                    count += 1;
                    return count;
                }
            );
        ...
        parse: {
            acceptance: function(mailBody, portal) {
                process.redacted.parse.lngLat(mailBody, portal);
                portal.resultTime = portal.confirmedTime;
                portal.status = value.code.portalStatus.accepted;
            },
            rejection: function(mailBody, portal) {
                process.redacted.parse.lngLat(mailBody, portal);
                portal.resultTime = portal.confirmedTime;
                if (mailBody.search(value.string.mail.keyword.redacted.rejectedReason.tooClose) > -1) {
                    portal.status = value.code.portalStatus.rejected.tooClose;
                } else if (mailBody.search(value.string.mail.keyword.redacted.rejectedReason.duplicated) > -1) {
                    portal.status = value.code.portalStatus.rejected.duplicated;
                } else {
                    portal.status = value.code.portalStatus.rejected.undeclared;
                }
            },
            lngLat: function(mailBody, portal) {
    ...
    prime: {
        ...
        start: function() {
            process.prime.confirmation();
            process.prime.acceptance();
            process.prime.rejection();
        },
        confirmation: function() {
            let count = 0;
            process.mails(
                { scanner: value.string.key.prime, type: value.string.key.confirmation },
                function(fullMail) {
                    portalList.push(process.parse.mail(fullMail));
                    count += 1;
                    return count;
        ...
        acceptance: function() {
            let count = 0;
            process.mails(
                { scanner: value.string.key.prime, type: value.string.key.acceptance },
                function(fullMail) {
                    portalList.push(process.parse.mail(fullMail, process.prime.parse.acceptance));
                    count += 1;
                    return count;
                }
            );
        ...
        rejection: function() {
            let count = 0;
            process.mails(
                { scanner: value.string.key.prime, type: value.string.key.rejection },
                function(fullMail) {
                    portalList.push(process.parse.mail(fullMail, process.prime.parse.rejection));
                    count += 1;
                    return count;
                }
            );
        ...
        parse: {
            acceptance: function(_mailBody, portal) {
                portal.resultTime = portal.confirmedTime;
                portal.status = value.code.portalStatus.accepted;
            },
            rejection: function(mailBody, portal) {
                portal.resultTime = portal.confirmedTime;
                portal.status = value.code.portalStatus.rejected.undeclared;
                for (let reason of Object.keys(value.string.mail.keyword.prime.rejectedReason)) {
                    for (let keyword of value.string.mail.keyword.prime.rejectedReason[reason]) {
                        if (mailBody.search(keyword) > -1) {
                            portal.status = value.code.portalStatus.rejected.tooClose;
                            break;
                        }
                    }
                    if (portal.status !== value.code.portalStatus.rejected.undeclared) break;
                }
            },
        },
    },
    mails: function(keys, onGetMail) {
        ...
        let progressUI = (keys.type === value.string.key.confirmation) ? ui.control.received : ui.control.portals[keys.type.ui];
    ...
    parse: {
        mail: function(fullMail, additionalProcessForMailBody) {
            let portal = {
                confirmedTime: fullMail.internalDate,
                status: value.code.portalStatus.pending
            };
