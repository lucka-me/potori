...
function displayPortals() {
    ...
    for (var i = 0; i < portalPendingList.length; i++) {
        ...
        portal.card = cardList.appendChild(newCard);
    }
...
function fillCardFinal(card, portal, iconElement) {
    ...
    iconElement.onclick = function() {scrollToCard(portal.bsId); };
...
function scrollToCard(bsId) {
    var cardList = document.getElementById("cardList");
    cardList.scrollTo(0, document.getElementById("card_" + bsId).offsetTop - cardList.offsetTop - 8);
}
