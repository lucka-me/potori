...
function processAcceptanceMails() {
    portalAcceptedList = [];
    processMails(mailAcceptanceList, portalAcceptedList, function(length) {
...
function processRejectionMails() {
    portalRejectedList = [];
    processMails(mailRejectionList, portalRejectedList, function(length) {
        psciRejectedCount.innerHTML = length + "/" + mailRejectionList.length;
    });
}

function processMails(mailList, portalList, finishSingleCallback, finishAllCallback) {
    for (var i = 0; i < mailList.length; i++) {
        ...
        request.execute(function(fullMail) {
            portalList.push(parseFullMail(fullMail));
...
function parseFullMail(fullMail) {
    var portalName = "";
    for (var i = 0; i < fullMail.payload.headers.length; i++) {
        var header = fullMail.payload.headers[i];
        if (header.name === "Subject") {
            portalName = header.value.replace("Portal submission confirmation: ", "").replace("Portal review complete: ", "");
            break;
        }
    }
    var portalImg = "";
    for (var i = 0; i < fullMail.payload.parts.length; i++) {
        ...
        if (part.partId == 1) {
            ...
            portalImg = mailBody.slice(
                mailBody.search(/googleusercontent\.com/),
                mailBody.search(/\" alt\=\"Portal /)
            );
            portalImg = portalImg.replace("googleusercontent.com/", "");
            break;
        }
    }
    return { name: portalName, url: portalImg, time: fullMail.internalDate };
}
...
function tryToShowCards() {
    ...
    for (var i = 0; i < portalAcceptedList.length; i++) {
        ...
        newCard.getElementById("statusIcon").className = "fas fa-check-circle fa-lg fa-fw status-accepted";
        newCard.getElementById("portalTitle").innerHTML = portal.name;
        newCard.getElementById("portalTitle").href = getBsWatermeterUrl(portal.url);
    ...
    for (var i = 0; i < portalPendingList.length; i++) {
        ...
        var newCard = document.getElementById("templateCard").content.cloneNode(true);
        newCard.getElementById("statusIcon").className = "fas fa-clock fa-lg fa-fw status-pending";
        newCard.getElementById("portalTitle").innerHTML = portal.name;
        newCard.getElementById("portalTitle").href = getBsWatermeterUrl(portal.url);
        newCard.getElementById("portalImg").src = imagePath + portal.url;
    ...
    for (var i = 0; i < portalRejectedList.length; i++) {
        ...
        var newCard = document.getElementById("templateCard").content.cloneNode(true);
        newCard.getElementById("statusIcon").className = "fas fa-times-circle fa-lg fa-fw status-rejected";
        newCard.getElementById("portalTitle").innerHTML = portal.name;
        newCard.getElementById("portalTitle").href = getBsWatermeterUrl(portal.url);
        newCard.getElementById("portalImg").src = imagePath + portal.url;
        newCard.getElementById("portalConfirmedTime").innerHTML = getDateString(portal.confirmedTime);
