const dialogKit = {
    status: {
        ...
        switchType: (type) => {
            ...
            const portals = process.portals.filter(
                portal => portal.status === value.data.type[type].code
            );
        ...
        switchRejecteReason: (reason) => {
            ...
            const hidden = !filter.switch.checked;
            const portals = process.portals.filter(
                portal => portal.status === value.data.rejectedReason[reason].code
            );
            for (const portal of portals) {
                document.getElementById(`card-${portal.id}`).hidden = hidden;
            }
            mapKit.updateRejectedData();
            if (!hidden && !dialogKit.status.filter.type.rejected.switch.checked) {
                dialogKit.status.filter.type.rejected.switch.checked = true;
        ...
        update: () => {
            ...
            for (const key of Object.keys(dialogKit.status.filter.type)) {
                dialogKit.status.filter.type[key].label.innerHTML =
                    toolkit.getCountString(counter.type[key], process.portals.length);
            }
            for (const key of Object.keys(dialogKit.status.filter.rejectedReason)) {
                dialogKit.status.filter.rejectedReason[key].label.innerHTML =
...
<<<<<<< HEAD
                    ...
                    filters.type[keys.type].label.innerHTML = toolkit.getCountString(filters.type[keys.type].portals, process.portals);
                    filters.type[selectedStatus].label.innerHTML = toolkit.getCountString(filters.type[selectedStatus].portals, process.portals);
...
>>>>>>> master
    ...
    import: {
        ...
        init: () => {
            dialogKit.import.ctrl.listen('MDCDialog:opened', dialogKit.import.onOpened);
