const dialogKit = {
    about: {
        ctrl: new mdc.dialog.MDCDialog(document.querySelector('#dialog-about')),
        init: () => {
            fetch(value.string.github.releaseUrl)
                .then(response => response.json())
                .then(response => {
                    dialogKit.about.ctrl.root_.querySelector('#text-version').innerHTML =
                        `${response.name}d${value.data.version}-${versionKit.code}`;
                });
        },
    },
    details: {
        ...
        map: {
            ...
            search: () => {
                ...
                dialogKit.details.map.button.search.root_.disabled = true;
                bsKit.queryLngLat(
                    dialogKit.details.data.portal.id, onSuccess, onFailed
                );
            },
            delete: () => {
                const map = dialogKit.details.map;
        ...
        init: () => {
            ...
            dialogKit.details.reasonSelect = new mdc.select.MDCSelect(reasonSelectElement);
            dialogKit.details.reasonSelect.selectedIndex = 0;
            ...
            const onStatusRadioChange = (event) => {
                ...
                dialogKit.details.reasonSelect.root_.hidden = !(event.target.value === 'rejected');
                dialogKit.details.reasonSelect.layout();
                map.ctrl.resize();
            };
        ...
        onOpened: () => {
            ...
            dialogKit.details.map.button.search.root_.disabled = false;
            ...
            if (keys.type === 'rejected') {
                dialogKit.details.reasonSelect.selectedIndex = portal.status - value.data.type.rejected.code;
            }
        ...
        onClosed: (event) => {
            ...
            if (event.detail.action === 'save') {
                ...
                const reason = dialogKit.details.reasonSelect.value;
                ...
                if (shouldUpdate) {
                    if (dialogKit.details.selectedStatus !== 'rejected') {
                        ...
                        portal.status = value.data.reason[dialogKit.details.reasonSelect.value].code;
                    }
        ...
        show: (portal) => {
            ...
            dialogKit.details.ctrl.root_.querySelector('#select-dialog-details-reason').hidden = !(keys.type === 'rejected');
    ...
    import: {
        ...
        init: () => {
            dialogKit.import.ctrl.listen('MDCDialog:closed', dialogKit.import.onClosed);
        },
        show: () => {
            dialogKit.import.ctrl.open();
        },
        onClosed: (event) => {
            if (event.detail.action === 'import') {
                process.import(dialogKit.import.textarea.value);
            }
        },
    },
    alert:  {
        ...
        show: (message) => {
            ...
            dialogKit.alert.ctrl.root_.querySelector('#box-alert-message').innerHTML = message;
        },
    },
    init: () => {
        dialogKit.about.init();
