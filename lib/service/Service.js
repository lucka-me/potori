...
class Parser {
    ...
    static bsData(content) {
        const result = { matched: false, message: '' };
        try {
            service.bs.data = new Map(JSON.parse(content));
            result.matched = true;
        } catch (error) {
            result.message = value.string.alert.openFileParseError;
        }
...
class BlobGenerator {
    ...
    static bsData() {
        return new Blob(
            [JSON.stringify([...service.bs.data], null, 4)],
            { type: value.string.file.type }
        );
    }
}
...
class Service {
    ...
    save() {
        ...
        this.file.local.save(value.string.file.portals, BlobGenerator.portals());
        window.setTimeout(() => {
            this.file.local.save(value.string.file.bsData, BlobGenerator.bsData());
        }, 2000);
    }
