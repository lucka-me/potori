...
class Service {
    ...
    save() {
        if (this.portals.length < 1) {
            this.event.alert(value.string.alert.saveFileNoPortal);
            return;
        }
    ...
    upload() {
        ...
        const checkFinish = () => {
            if (uploadedPortals && uploadedBsData) {
                this.event.alert(value.string.alert.uploaded);
            };
        };
        ...
        this.file.googleDrive.uploaded(
            ...
            (response) => {
                ...
                if (!response) {
                    this.event.alert(value.string.alert.uploadFailed);
                } else if (!response.id) {
                    this.event.alert(`${value.string.alert.uploadFailed}\n${response.message}`);
                    return;
                }
    ...
    import(raw) {
        ...
        try {
            ...
            this.event.alert(value.string.alert.invalidData);
            return;
        }
    ...
    updateBsData() {
        this.bs.update(this.portals, () => {
            this.event.updateBs();
            this.event.alert(value.string.alert.updated);
        });
    }
}
