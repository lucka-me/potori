        ...
        submittions: {
            ...
            update: () => {
                if (process.portals.length === 0) {
                    dashboard.card.submittions.chart.data.datasets[0].data = [];
                ...
                let scan = min;
                while (scan < max) {
                    const month = moment(scan).startOf('month');
                    const key = month.valueOf();
                    if (!dataMap.has(key)) dataMap.set(key, 0);
                    scan = month.endOf('month').valueOf() + 1000;
                }
                const data = [];
