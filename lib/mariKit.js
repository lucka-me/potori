const mariKit = {
    ...
    start() {
        ...
        const downloaded = () => {
            ...
            this._ignoreMailIdList  = [];
            for (const portal of process.portals) {
                this._ignoreMailIdList.push(portal.confirmationMailId);
                if (portal.resultMailId) this._ignoreMailIdList.push(portal.resultMailId);
            }
            for (const scanner of value.string.key.scanner) {
                for (const type of value.string.key.type) {
                    this.mails({ scanner: scanner, type: type });
                }
            }
        };
        process.download(downloaded);
    },
    mails(keys) {
        ...
        const requestListHandler = (response) => {
            ...
            if (response.result.nextPageToken) {
                ...
                for (let i = list.length - 1; i >= 0; i--) {
                    for (let mailId of this._ignoreMailIdList) {
                        if (list[i].id === mailId) {
                            list.splice(i, 1);
                            break;
                        }
                ...
                ui.progress.ctrl.buffer = this.progress.list / 6;
        ...
        const processMailList = (list) => {
            ...
            for (const mail of list) {
                ...
                request.execute((fullMail) => {
                    process.portals.push(this.parse.mail(fullMail, keys));
                    this.progress.finished += 1;
                    ui.progress.ctrl.progress =
                        this.progress.finished / this.progress.total * (this.progress.list / 6) * 0.9;
                    checkFinish();
                });
            }
        };
