const process = {
    ...
    save: () => {
        if (process.portals.length < 1) {
            dialog.alert.show(value.string.alert.saveFileNoPortal);
            return;
        }
    ...
    upload: () => {
        ...
        const checkFinish = () => {
            if (uploadedPortals && uploadedBsData) {
                dialog.alert.show(value.string.alert.uploaded);
            };
        };
        ...
        fileKit.googleDrive.uploaded(
            ...
            (response) => {
                ...
                if (!response) {
                    dialog.alert.show(value.string.alert.uploadFailed);
                } else if (!response.id) {
                    dialog.alert.show(`${value.string.alert.uploadFailed}\n${response.message}`);
                    return;
                } else {
    ...
    import: (raw) => {
        ...
        try {
            ...
            dialog.alert.show(value.string.alert.invalidData);
            return;
        }
    ...
    updateBsData: () => {
        bsKit.update(() => {
            dashboard.card.bs.update();
            dialog.alert.show(value.string.alert.updated);
        });
    },
};