window.process = {
    ...
    save: () => {
        if (process.portals.length < 1) {
            dialog.alert.open(value.string.alert.saveFileNoPortal);
            return;
        }
    ...
    upload: () => {
        ...
        const checkFinish = () => {
            if (uploadedPortals && uploadedBsData) {
                dialog.alert.open(value.string.alert.uploaded);
            };
        };
        ...
        fileKit.googleDrive.uploaded(
            ...
            (response) => {
                ...
                if (!response) {
                    dialog.alert.open(value.string.alert.uploadFailed);
                } else if (!response.id) {
                    dialog.alert.open(`${value.string.alert.uploadFailed}\n${response.message}`);
                    return;
                } else {
    ...
    import: (raw) => {
        ...
        try {
            ...
            dialog.alert.open(value.string.alert.invalidData);
            return;
        }
    ...
    updateBsData: () => {
        bsKit.update(() => {
            dashboard.bs.update();
            dialog.alert.open(value.string.alert.updated);
        });
    },
};