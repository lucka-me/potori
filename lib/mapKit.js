const mapKit = {
    ...
    updateData: () => {
        ...
        mapKit.updateSource(
            'accepted',
            mapKit.generateGeoJSON(dialogKit.status.filter.type.accepted.portals)
        );
        mapKit.updateSource(
            'pending',
            mapKit.generateGeoJSON(dialogKit.status.filter.type.pending.portals)
        );
    },
    updateRejectedData: () => {
        let portals = [];
        for (const key of Object.keys(dialogKit.status.filter.rejectedReason)) {
            if (!dialogKit.status.filter.rejectedReason[key].switch.checked) continue;
            portals = [...portals, ...dialogKit.status.filter.rejectedReason[key].portals];
        }
        mapKit.updateSource('rejected', mapKit.generateGeoJSON(portals));
    },
