...
<<<<<<< HEAD
        for (const key of Object.keys(dialogKit.status.filter.type)) {
            dialogKit.status.filter.type[key].label.innerHTML = toolkit.getCountString(dialogKit.status.filter.type[key].portals, process.portals);
            dialogKit.status.filter.type[key].switch.checked = true;
        }
        for (const key of Object.keys(dialogKit.status.filter.rejectedReason)) {
            ...
            filter.label.innerHTML = toolkit.getCountString(filter.portals, dialogKit.status.filter.type.rejected.portals);
            filter.switch.checked = true;
=======
...
>>>>>>> master
        ...
        const onFinished = () => {
            ...
            ui.appBar.button.import.root_.hidden = false;
            ui.progressBar.root_.hidden = true;
        };
